...
for (let index = 0; index < PredictedArray.length; index++) {
  ErrorArray.push(
    (PredictedArray[index] - Y[index]) * (PredictedArray[index] - Y[index])
  );
}
...
var out = fs.createWriteStream("DollarsDataReport.txt", { flags: "a" });

//M
for (let indexM = 0; indexM < 5; indexM += 0.01) {
  //C
  for (let indexC = -6000; indexC < -4000; indexC += 0.05) {
    //X
